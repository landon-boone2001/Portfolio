...
void Dungeon::readRooms(string roomFile) {
    ifstream fin;
    totalRooms = 0;
    string input;
    string inFile = roomFile;
    string tempString;
    char direction;
    int exitNum = 0;
    int  i, j = 0;
...
void Dungeon::readEncounters(string encounterFile) {
    ifstream fin;
    totalEncounters = 0;
    string input;
    string inFile = encounterFile;
    int i =0;
    fin.open(inFile);
    while (getline(fin, input)) {
        if (input.compare("~") == 0) {
...
void Dungeon::readEnemyList(string enemyFile) {
    ifstream fin;
    totalEnemies = 0;
    string input;
    string inFile = enemyFile;
    int i = 0;
    fin.open(inFile);
    while (getline(fin, input)) {
        if (input.compare("~") == 0) {
...
int main(int argc, char **argv) {
    ...
    if (argc != 4) {
        cerr << "Usage: ./dungeonEscape roomList.txt encounterList.txt enemyList.txt" << endl;
        return 1;
    }
    Dungeon dungeon;
    dungeon.readRooms(argv[1]);
    dungeon.readEncounters(argv[2]);
    dungeon.readEnemyList(argv[3]);
    int rollCheck = 0;
    bool enemyTurn;
